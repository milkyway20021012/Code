...
public:
  ...
  // string() : _size(0), _capacity(0), _str(new char[1]) // 至少要開闢一個 '/0'
  // 的空間 { // 無參
  //     _str[0] = '\0';
  // }
  ...
  string(const char *str = "") // 带参
  {
    ...
    /*
    之所以使用memcpy而不是strcpy是因爲strcpy只會複製'/0'之前的的字符
    如果字符串是一個沒有'/0'的字符串則會有越界的問題
    */
  }
  ...
  string(string &&s) {
    cout << "string(string&& s) ---- 移动拷贝构造调用" << endl;
    swap(s);
  }
  ~string() {
    delete[] _str;
  ...
  char &
  operator[](size_t pos) // 重載運算符 讓string也可以像C char 一樣用下標訪問
  {
    assert(pos < _size);
    return _str[pos];
  }
  ...
  void insert(size_t pos, size_t n, char ch) {
    ...
    while (end >= pos &&
           end != npos) /* 可以嘗試(int) 強轉pos類型 => 要轉是因爲 end 為int
                 pos為size_t 會發生類型提升(提升至較大的類型 : size_t)
                 仍然會陷入死循環
              */
    {
      _str[end + n] = _str[end];
  ...
  string &operator=(string &&s) {
    cout << "string operator=(string&& s) --- 移动赋值调用" << endl;
    swap(s);
    return *this;
  }
